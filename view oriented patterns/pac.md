...
9. **Provide an extrernal interface** - To cooperate with other agents every agent send and receives events and data. Implement this functionality as part of the **control** component. Within and agent events and data are forwarded to their intended recipients. Those recipients can be another agents. In that case the component is a mediator and you may use [Mediator pattern](https://en.wikipedia.org/wiki/Mediator_pattern).
